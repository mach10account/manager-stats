...
- **Churn** (Sauron → Delivery) = coorte di **CONTRATTI IN SCADENZA**: dei contratti con `fine_servizio` nel mese, quanti NON hanno un contratto successivo (stesso centro, qualunque tag) creato entro **fine + 60 giorni**. Base contratti e non clienti perché su `centri` la FINE SERVIZIO al rinnovo non viene aggiornata in 3 casi su 4 e l'aggancio per nome perde il 19% dei contratti. La tolleranza c'è perché il rinnovo si firma in media **30 giorni dopo** la scadenza: senza, il churn passa da 85% a 93% e misura la burocrazia. ⚠️ Gli ultimi 2 mesi non hanno finito la tolleranza → churn provvisorio, può solo scendere.
