...
- **P&L**: i **ricavi sono SEMPRE gli incassi Notion** (rate con data incasso nel mese) — regola di Leo 7/8. Dal foglio (`fin_pnl`) si prendono **solo i costi e gli storni** dei mesi che ci stanno; il cash dichiarato sul foglio resta nel pannello di confronto e basta. Mese corrente = costi dal registro. `fin_costi` è lo stato di OGGI, inutile sul passato.
- **Churn** (Sauron → Delivery) = coorte di **CONTRATTI IN SCADENZA**: dei contratti con `fine_servizio` nel mese, quanti NON hanno **mai** avuto un contratto successivo (stesso centro, qualunque tag). **Senza limiti di tempo** — decisione di Leo 7/8: un cliente che rifirma anche sei mesi dopo non è perso, e il churn del mese in cui era scaduto scende. Conseguenza accettata: nessun mese si congela davvero. Base contratti e non clienti perché su `centri` la FINE SERVIZIO al rinnovo non viene aggiornata in 3 casi su 4 e l'aggancio per nome perde il 19% dei contratti. ⚠️ I mesi scaduti da meno di 60 giorni (`GRAZIA_RINNOVO`, ora usata **solo** per marcarli) hanno quasi tutti i rinnovi ancora da firmare — la firma arriva in media 30 giorni dopo — e vanno letti come un tetto.
- Da non confondere con la tile **"Clienti persi"**, che conta la `DATA CLIENTE PERSO` nel mese: è quando li abbiamo *segnati* persi (in media 4 mesi dopo la fine servizio), non quando se ne sono andati — e metà dei persi non ce l'ha compilata.
